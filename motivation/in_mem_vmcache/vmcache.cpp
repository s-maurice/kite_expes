...
struct BufferManager {
  ...
  bool isValidPtr(void *page) {
    return (page >= virtMem) && (page < (virtMem + memSize + 16));
  }
...
BufferManager::BufferManager()
    : memSize(envOr("MEMGB", 16) * gb), virtCount(memSize / pageSize),
      physCount(memSize / pageSize), residentSet(physCount) {
  u64 virtAllocSize = memSize + (1 << 16); // we allocate 64KB extra to prevent
...
public:
  ...
  BTreeNode *findLeafNoGuard(span<u8> key) {
    MetaDataPage *meta =
        reinterpret_cast<MetaDataPage *>(bm.toPtr(metadataPageId));
    BTreeNode *node =
        reinterpret_cast<BTreeNode *>(bm.toPtr(meta->getRoot(slotId)));

    while (node->isInner())
      node = reinterpret_cast<BTreeNode *>(bm.toPtr(node->lookupInner(key)));
    return node;
  }

  template <class Fn> bool lookupNoGuard(span<u8> key, Fn fn) {
    BTreeNode *node = findLeafNoGuard(key);
    bool found;
...
int main(int argc, char **argv) {
  ...
  u64 runForSec = envOr("RUNFOR", 10);
  bool isRndread = envOr("RNDREAD", 0);
  bool useGuards = envOr("USE_GUARDS", 1);
  bool isZipfian = envOr("ZIPFIAN", 0);
  double zipfianTheta = envOrDouble("ZIPF_THETA", 0.99);
  ...
  auto statFn = [&]() {
    cout << "ts,tx,threads,datasize" << endl;
    u64 cnt = 0;
    for (uint64_t i = 0; i < runForSec; i++) {
      sleep(1);
      u64 prog = txProgress.exchange(0);
      cout << cnt++ << "," << prog << "," << nthreads << "," << n << endl;
    }
    keepRunning = false;
  };
  ...
  ZipfianGenerator zipf(isZipfian ? n : 1, zipfianTheta);
  ...
  parallel_for(0, nthreads, nthreads,
               [&](uint64_t worker, uint64_t begin, uint64_t end) {
                 ...
                 while (keepRunning.load()) {
                   ...
                   if (isZipfian) {
                     v1 = __builtin_bswap64(zipf.getNext());
                   } else {
                     v1 =
                   ...
                   if (useGuards) {
                     ...
                     succ =
                         bt.lookupNoGuard({k1, sizeof(u64)}, [&](span<u8> p) {
                           // memcpy(payload.data(), p.data(), p.size());
                         });
                   }
                   assert(succ);
